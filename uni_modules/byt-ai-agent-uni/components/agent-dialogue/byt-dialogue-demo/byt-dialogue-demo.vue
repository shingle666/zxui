<template>
	<view class="byt-aidialogue" :class="{ 'is-default-bg': !config.main.background }" :style="aidialogueStyle">
		...
		<view class="byt-aidialogue-bg">
			...
			<view class="byt-aidialogue-header">
				...
				<!-- <Prompts :colorIcon="colorIconAgent" @itemClick="problemClick" v-if="showRecommendedMessages"></Prompts> -->
				<transition name="fade">
					<Prompts :colorIcon="colorIconAgent" @itemClick="problemClick" v-if="showRecommendedMessages"
            ...
            ></Welcome>
			    <Prompts :colorIcon="colorIconAgent"  @itemClick="problemClick"></Prompts> -->
			<!-- 消息列表  -->
			<view class="byt-aidialogue-content byt-aidialogue-bg--float" :style="{
				...
				<BubbleList :colorIcon="colorIconAgent" :typeWhiteList="config.typeWhiteList" :roles="config.roles"
					...
					<!-- <template v-slot:content="{ type }">
                        <Prompts :colorIcon="colorIconAgent" v-if="type == 3 && config.guess" @itemClick="problemClick"></Prompts>
                    </template> -->
					<template #footer>
	...
	export default {
			...
			bubbleItems: function() {
				const list = this.recordList.map((item) => {
					return {
						...
						// 深度思考状态
						deepSeekStatus: item.deepSeekStatus || false,
						// 深度思考内容
						deepSeekContent: item.deepSeekContent || '',
						// 深度思考时间
						deepSeekTime: item.deepSeekTime || 0,
		...
		methods: {
			...
			getData(stopPullDownRefresh = false) {
				...
				// 模拟
				chatHistoryListMook(this.queryParams).then((data) => {
			...
			connectSocketInit() {
				...
				_this.socketTask = WebSocketMook();
				...
				_this.socketTask.onMessage(_this.messageProcess);
			},
			// 处理返回消息
			messageProcess(res) {
				const _this = this;
				...
				const closeDeepSeek = () => {
					if (_this.deepSeek) {
						_this.deepSeek = false;
						clearInterval(_this.deepSeekSetTimes);
						_this.recordList[lastIndex].deepSeekTime = _this.deepSeekTime;
						_this.recordList[lastIndex].deepSeekStatus = _this.deepSeek;
					}
				};
				...
				if (res.data == tagList[0]) {
					...
					if (_this.answerImgSrc) {
						_this.recordList[lastIndex].content = _this.recordList[lastIndex].content.replace('......', _this
							.answerImgSrc);
						_this.answerImgSrc = '';
					}
					// #endif
					...
					_this.deepSeek = true;
					_this.recordList[lastIndex].deepSeekStatus = _this.deepSeek;
					_this.deepSeekTime = 0;
					_this.deepSeekSetTimes = setInterval(() => {
						_this.deepSeekTime++;
					}, 1000);
				} else {
					if (_this.deepSeek) {
						const deepSeekContentStr = _this.recordList[lastIndex].deepSeekContent || '';
						// 深度思考
						_this.recordList[lastIndex].deepSeekContent = deepSeekContentStr + res.data;
			...
			async setMessage() {
				const _this = this;
				// _this.showRecommendedMessages = false
				...
				_this.socketTask.send({
						data: message,
						async success() {
							console.log('消息发送成功11');
						}
					},
					_this.activeList.includes(1)
				);
				//模拟回答中动画
				_this.recordList.push({
					createTime: getCurrentDateTime(),
...
<style lang="scss" scoped>
	...
	@include b(aidialogue) {
		...
		@include b(aidialogue-header) {
			// background:url('https://img.yummall.cn/baiyetong/byt-uni-ui/headerBgimg.svg') ;
			// background-size: 100% 100%;

			// background-image: url(https://img.yummall.cn/baiyetong/byt-uni-ui/headerBgimg.svg);
