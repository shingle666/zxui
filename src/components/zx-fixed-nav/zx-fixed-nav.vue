<template>
  <view class="zx-fixed-nav" :class="{
    'zx-fixed-nav--active': visible,
    'zx-fixed-nav--left': type === 'left'
  }" :style="navStyle">
    <!-- 遮罩层 -->
    <view v-if="overlay && visible" class="zx-fixed-nav__overlay" @click="handleOverlayClick"></view>

    <!-- 导航列表 -->
    <view class="zx-fixed-nav__list" :style="listStyle">
      <view v-for="(item, index) in navList" :key="item.id || index" class="zx-fixed-nav__list-item"
        :class="{ 'zx-fixed-nav__list-item--active': item.id === activeId }" @click="handleItemClick(item, $event)">
...
const iconName = computed(() => {
  if (props.type === 'left') {
    return props.visible ? 'arrow-left' : 'arrow-right'
  } else {
    return props.visible ? 'arrow-right' : 'arrow-left'
  }
})
