<template>
  <view class="zx-ellipsis" :class="classNames" :style="containerStyle" @click="handleClick">
    ...
    <text 
      v-if="rows === 1" 
      class="zx-ellipsis__text zx-ellipsis__text--single" 
      :style="textStyle"
    >
      {{ displayContent }}
    </text>
    ...
    <view v-else class="zx-ellipsis__multiline" :style="multilineStyle">
      <text 
        class="zx-ellipsis__text zx-ellipsis__text--multiline" 
        :style="textStyle"
      >
        {{ displayContent }}
      </text>
      ...
      <!-- 展开/收起按钮 -->
      <text 
        v-if="showToggle && (expandText || collapseText)" 
        class="zx-ellipsis__toggle" 
        :style="toggleStyle"
