<template>
	<view class="zx-navbar">
		<!-- 固定在顶部时的占位 -->
		<view v-if="fixed && placeholder" class="zx-navbar__placeholder" :style="{ height: barHeight+'px', backgroundColor: bgColor }"></view>
		<view :class="[fixed && 'zx-navbar--fixed']">
			<zx-status-bar v-if="safeAreaInsetTop" :backgroundColor="bgColor"></zx-status-bar>
...
onMounted(() => {
	...
	barHeight.value = sys.statusBarHeight + 44;
});
