<template>
	<view class="zx-goods-price" :class="[`zx-goods-price--${size}`, { 'zx-goods-price--line-through': lineThrough }]" :style="priceStyle">
		<!-- 价格前缀 -->
		<slot name="prefix">
			<text v-if="prefix" class="zx-goods-price__prefix" :style="{ color: prefixColor, fontSize: prefixSize + 'rpx' }">
				{{ prefix }}
			</text>
		</slot>
		...
		<!-- 整数部分 -->
		<text class="zx-goods-price__integer" :style="{ color: color, fontSize: integerSize + 'rpx', fontWeight: fontWeight }">
			{{ integerPart }}
		</text>
		
		<!-- 小数部分 -->
		<text v-if="showDecimal && decimalPart" class="zx-goods-price__decimal" :style="{ color: color, fontSize: decimalSize + 'rpx' }">
			.{{ decimalPart }}
		</text>
		
		<!-- 价格后缀 -->
		<slot name="suffix">
			<text v-if="suffix" class="zx-goods-price__suffix" :style="{ color: suffixColor, fontSize: suffixSize + 'rpx' }">
				{{ suffix }}
			</text>
		</slot>
	</view>
