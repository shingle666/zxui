<template>
	<view class="zx-cropper__box" @touchmove.stop.prevent="stop">
		<image @load="imageLoad" @error="imageLoad" @touchstart="onTouchStart" @touchmove="onTouchMove"
			@touchend="onTouchEnd" :data-minScale="minScale" :data-maxScale="maxScale" :style="{
		...
		<view class="zx-backdrop__cropper"
			...
			<view class="zx-cropper__border" :style="{ borderRadius: isRound ? '50%' : '0', border: border }"></view>
		</view>
...
<script setup>
import { ref, watch, computed, nextTick, onMounted, defineEmits, defineProps, getCurrentInstance } from 'vue';
...
function imageReset() {
	scale.value = 1;
	angle.value = 0;
	let sys = sysInfo.value.windowHeight ? sysInfo.value : uni.getSystemInfoSync();
