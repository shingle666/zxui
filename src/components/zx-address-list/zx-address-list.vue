<template>
  <view class="zx-address-list">
    <slot name="top"></slot>
    <radio-group v-if="singleChoice && list && list.length > 0" @change="onRadioChange">
    ...
    <checkbox-group v-else-if="!singleChoice && list && list.length > 0" @change="onCheckboxChange">
       <view v-for="(item, index) in list" :key="item.id">
        <zx-address-list-item
          :address="item"
          :single-choice="singleChoice"
...
import ZxAddressListItem from './zx-address-list-item.vue'; // Assuming a local item component
import ZxButton from '../zx-button/zx-button.vue'; // Assuming zx-button component
