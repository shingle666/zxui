<template>
	<view class="zx-date-picker" :class="[`zx-date-picker--${type}`, { 'zx-date-picker--disabled': disabled }]">
		...
		<view class="zx-date-picker__input-wrapper" @click="handleOpen">
			<view class="zx-date-picker__input" :class="{ 'zx-date-picker__input--focus': visible }">
				...
				<view class="zx-date-picker__inner">
					<text 
						v-if="displayValue" 
						class="zx-date-picker__text"
						:style="{ color: textColor }"
					>{{ displayValue }}</text>
					<text 
						v-else 
						class="zx-date-picker__placeholder"
					>{{ placeholder }}</text>
				</view>
		...
		<view 
			v-if="visible" 
			class="zx-date-picker__mask" 
			:class="{ 'zx-date-picker__mask--show': visible }"
			@click="handleMaskClick"
		></view>
		
		<view 
			v-if="visible"
			class="zx-date-picker__popup" 
			:class="{ 'zx-date-picker__popup--show': visible }"
		...
		>
			...
			<view v-if="shortcuts.length > 0" class="zx-date-picker__shortcuts">
				<view 
					...
					:key="index"
					class="zx-date-picker__shortcut"
				...
				<template v-if="type === 'date' || type === 'datetime'">
					...
					<view class="zx-date-picker__week-header">
						<view 
							...
							:key="index"
							class="zx-date-picker__week-title"
						>
							<text>{{ week }}</text>
						</view>
					</view>
					...
					<view v-if="type === 'datetime'" class="zx-date-picker__time">
						<picker-view 
							class="zx-date-picker__time-picker"
							:value="timeValue"
						...
						>
							<picker-view-column>
								<view 
									v-for="hour in hours"
									:key="hour"
									class="zx-date-picker__time-item"
				...
				<template v-else-if="type === 'daterange' || type === 'datetimerange'">
					<view class="zx-date-picker__range">
						...
						<view class="zx-date-picker__panel zx-date-picker__panel--left">
							...
							<view class="zx-date-picker__dates">
								<view 
									v-for="(date, index) in leftDates"
									:key="index"
									class="zx-date-picker__date"
									:class="getRangeDateClass(date, 'left')"
									@click="handleRangeDateClick(date, 'left')"
								>
									<text class="zx-date-picker__date-text">{{ date.day }}</text>
								</view>
							</view>
						...
						<view class="zx-date-picker__panel zx-date-picker__panel--right">
							...
							<view class="zx-date-picker__dates">
								<view 
									v-for="(date, index) in rightDates"
									:key="index"
									class="zx-date-picker__date"
									:class="getRangeDateClass(date, 'right')"
									@click="handleRangeDateClick(date, 'right')"
								>
									<text class="zx-date-picker__date-text">{{ date.day }}</text>
								</view>
							</view>
...
const props = defineProps({
	...
	type: {
		...
		default: 'date',
		validator: (value) => {
			return ['year', 'month', 'date', 'datetime', 'daterange', 'datetimerange'].includes(value);
		}
	},
	// 输入框占位文本
	placeholder: {
...
const getRangeDateClass = (dateItem, panel) => {
	...
	if (rangeValue.value.length > 0) {
		...
		if (rangeValue.value.length === 2 && isInRange(dateItem.date, rangeValue.value[0], rangeValue.value[1])) {
...
const getMonthClass = (monthItem) => {
	...
	if (monthItem.value === new Date().getMonth() + 1 && 
...
const handleConfirm = () => {
	if (isRange.value) {
		if (tempRangeValue.value.length === 2) {
			...
			const formattedValue = tempRangeValue.value.map(date => formatDate(date, getValueFormat()));
...
const handleDateClick = (dateItem) => {
	if (!dateItem.isCurrentMonth || (props.disabledDate && props.disabledDate(dateItem.date))) {
		return;
	}
...
const prevYear = (panel = 'single') => {
	if (panel === 'single') {
		...
		leftPanelDate.value = new Date(leftPanelDate.value.getFullYear() - 1, leftPanelDate.value.getMonth(), 1);
	} else if (panel === 'right') {
		rightPanelDate.value = new Date(rightPanelDate.value.getFullYear() - 1, rightPanelDate.value.getMonth(), 1);
	}
...
const prevMonth = (panel = 'single') => {
	if (panel === 'single') {
		...
		leftPanelDate.value = new Date(leftPanelDate.value.getFullYear(), leftPanelDate.value.getMonth() - 1, 1);
	} else if (panel === 'right') {
		rightPanelDate.value = new Date(rightPanelDate.value.getFullYear(), rightPanelDate.value.getMonth() - 1, 1);
	}
		...
		if (isRange.value && Array.isArray(props.modelValue)) {
			...
			if (rangeValue.value.length > 0) {
				...
				if (rangeValue.value.length > 1) {
					...
					rightPanelDate.value = new Date(leftPanelDate.value.getFullYear(), leftPanelDate.value.getMonth() + 1, 1);
				}
			}
		} else if (!isRange.value) {
