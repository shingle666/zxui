<template>
  <view class="zx-goods-menu">
    <!-- 菜单栏 -->
    <view 
      class="zx-goods-menu__bar" 
      :class="{ 'zx-goods-menu__bar--fixed': scrollFixed }"
      :style="{ zIndex: scrollFixed ? scrollFixedZIndex : 'auto' }"
    >
      <view 
        v-for="(item, index) in menuItems" 
        :key="index"
        class="zx-goods-menu__item"
        :class="{ 
      ...
      >
        <view class="zx-goods-menu__title">
          ...
          <text 
            v-if="!item.disabled" 
            class="zx-goods-menu__arrow"
            :class="{ 'zx-goods-menu__arrow--up': activeIndex === index && direction === 'down' }"
          ></text>
        </view>
      </view>
    </view>
    ...
    <!-- 遮罩层 -->
    <view 
      v-if="overlay && showPopup" 
      class="zx-goods-menu__overlay"
    ...
    >
      <view class="zx-goods-menu__wrapper">
        <!-- 自定义内容 -->
        <slot 
          v-if="activeItem && activeItem.options"
          name="content" 
          :item="activeItem" 
        ...
        >
          <!-- 默认选项列表 -->
          <view 
            class="zx-goods-menu__options"
            :style="{ gridTemplateColumns: `repeat(${cols}, 1fr)` }"
          >
            <view 
              v-for="(option, optionIndex) in activeItem.options" 
              :key="optionIndex"
              class="zx-goods-menu__option"
            ...
            >
              ...
              <text 
                v-if="option.value === activeItem.value" 
                class="zx-goods-menu__option-check"
              ></text>
            </view>
          </view>
        </slot>
