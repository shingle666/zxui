	...
	<view class="zx-rate" :id="elId" ref="zx-rate" :style="[customStyle]">
		<view class="zx-rate__content" @touchmove.stop="touchMove" @touchend.stop="touchEnd">
			<view class="zx-rate__content__item" v-for="(item, index) in Number(max)" :key="index" :class="[elClass]">
				<view class="zx-rate__content__item__icon-wrap" ref="zx-rate__content__item__icon-wrap" @tap.stop="clickHandler($event, index + 1)">
					<zx-icon
...
const getIconName = (index) => {
	if (currentValue.value > index) {
		...
		if (Array.isArray(props.icons)) {
			...
			// 对象形式的icons配置
			const keys = Object.keys(props.icons).map(Number).sort((a, b) => a - b);
			for (let key of keys) {
...
const getRateItemRect = async () => {
	...
	uni.createSelectorQuery().in(proxy).select(`#${elId}`).boundingClientRect((res) => {
		if (res) {
...
const getRateIconWrapRect = () => {
	uni.createSelectorQuery().in(proxy).select(`.${elClass}`).boundingClientRect((res) => {
		if (res) {
