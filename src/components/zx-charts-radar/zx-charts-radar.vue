<template>
	<view class="zx-charts__radar-box" :style="{ width: radar_w + 'px' }">
		...
		<view class="zx-charts-radar" :class="{ 'zx-radar__mrgin': label.show }" :style="{ width: radar_w + 'px', height: radar_w + 'px' }">
			<view
			...
			>
				<view class="zx-radar__name" v-if="label.show" :style="{ color: label.color || '#bbb', fontSize: (label.size || 24) + 'rpx' }">{{ item.name }}</view>
			</view>
