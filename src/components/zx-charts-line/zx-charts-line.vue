		...
		<view class="zx-charts__line-box" v-if="xAxis.length > 0 && dataset.length > 0" :style="{ width: width + 'rpx' }">
			<scroll-view :scroll-x="scrollable" class="zx-line__scroll-view" :style="{ height: scrollViewH + 'rpx' }">
				...
				<view class="zx-charts__line" :style="{ height: height + 'rpx' }">
					<view
						class="zx-line__item"
						:class="{ 'zx-line__flex-1': !scrollable }"
						:style="{ width: (xAxisLine.itemGap || 120) + 'rpx' }"
						v-for="(item, index) in xAxis"
						:key="index"
					>
						<view class="zx-line__xAxis-text" :style="{ color: xAxisLabel.color || '#333', fontSize: (xAxisLabel.size || 24) + 'rpx' }">
							{{ item }}
						</view>
						<view
							class="zx-yAxis__split-line"
						...
						></view>
						<view class="zx-xAxis__tickmarks" :style="{ height: xAxisTick.height || '12rpx', backgroundColor: xAxisTick.color || '#e3e3e3' }"></view>
					</view>
					<view v-for="(dot, i) in dots" :key="dot.id">
						<view
							class="zx-charts__line-dot"
							:class="{ 'zx-charts__dot-enlarge': tooltipShow && j == activeIdx }"
						...
						>
							<text class="zx-line__val" :style="{ fontSize: (xAxisVal.size || 24) + 'rpx', color: xAxisVal.color }" v-if="xAxisVal.show">
								{{ getYAxisVal(i, j) }}
							</text>
						</view>
					</view>
			...
			<view class="zx-line__border-left" :style="{ height: height + (xAxisVal.height || 48) + 'rpx', backgroundColor: yAxisLine.color || '#e3e3e3' }"></view>
			<view
				class="zx-xAxis__line"
				:class="{ 'zx-line__first': index === 0 }"
				:style="{
				...
				:key="index"
			>
				<text class="zx-yAxis__val" :style="{ color: item.color || yAxisLabel.color, fontSize: (yAxisLabel.size || 24) + 'rpx' }" v-if="yAxisLabel.show">
					{{ item.value }}
				</text>
			</view>
		</view>
...
const props = defineProps({
	...
	xAxisVal: { type: Object, default: () => ({ show: true, color: '#333', size: 24, height: 48 }) },
	yAxisSplitLine: { type: Object, default: () => ({ color: 'transparent', type: 'dashed' }) },
...
function getCoordinatePoint() {
	...
	const itemGap = props.scrollable ? props.xAxisLine.itemGap || 120 : props.width / xSections;
	let dotsArr = [];
	...
	dataset.value.map((item, index) => {
		...
		source.map((val, idx) => {
			dotArr.push({
				...
				y: getValue(((val - props.min) / (maxValue.value - props.min)) * (props.yAxisLine.itemGap || 60) * ySections - radius)
			});
		});
		dotsArr.push({
			id: 'dd' + index,
