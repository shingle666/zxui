						...
						>
							...
							<view class="zx-cascader__item-content">
								...
								<text 
									...
									:style="{ 
										fontSize: size + 'rpx', 
										color: isItemSelected(item) ? activeColor : (item[disabledKey] ? disabledColor : color)
									}"
								>{{ item[labelKey] }}</text>
...
const hasChildren = (item) => {
	return item && Array.isArray(item[childrenKey.value]) && item[childrenKey.value].length > 0;
};
...
const isItemSelected = (item) => {
	...
	return selectedPath.value[currentLevel.value] && 
...
const updateParentChecked = () => {
	...
	const updateParent = (options) => {
		options.forEach(item => {
			if (hasChildren(item)) {
				const children = item[childrenKey.value].filter(child => !child[disabledKey.value]);
