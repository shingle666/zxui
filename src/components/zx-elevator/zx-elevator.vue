<template>
  <view class="zx-elevator" :style="customStyle">
      ...
      <view class="zx-elevator__list-wrapper">
        ...
        >
          ...
          <view
            ...
            :key="subItem.id"
            class="zx-elevator__list-item__content"
            :class="{
    ...
    <!-- 吸顶标题 -->
    <view
      v-if="isSticky && scrollY > 0"
      class="zx-elevator__sticky-title"
    >
    ...
    <!-- 当前选中的索引提示 -->
    <view
      v-if="showCurrentIndex"
      class="zx-elevator__current-index"
    >
    ...
    >
      <view
        v-for="(item, index) in indexList"
        :key="item[acceptKey]"
        class="zx-elevator__sidebar-item"
        :class="{
          'zx-elevator__sidebar-item--active': index === currentIndex
        }"
        :data-index="index"
        @click="handleClickIndex(item[acceptKey], index)"
      >
        {{ item[acceptKey] }}
      </view>
    </view>
