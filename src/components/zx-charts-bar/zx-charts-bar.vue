<template>
	<view class="zx-charts__bar-wrap" :style="{ width: width + 'rpx' }">
		...
		<view class="zx-charts__bar-box" :style="{ borderLeftColor: yAxisLine.color, borderBottomColor: xAxisLineColor }" v-if="yAxis.length > 0 && dataset.length > 0">
			<view
			...
			>
				<view
					...
					:style="{
						...
						width: ((bar.source[index] - (isStack ? min / dataset.length : min)) / (maxValue - min)) * width + 'rpx'
					}"
				...
				<view class="zx-bar__yAxis-text" :style="{ color: yAxisLabel.color || '#333', fontSize: (yAxisLabel.size || 24) + 'rpx' }">
					{{ item }}
				</view>
				<view class="zx-yAxis__tickmarks" :style="{ width: yAxisTick.width || '12rpx', backgroundColor: yAxisTick.color || '#e3e3e3' }"></view>
			</view>
			<view class="zx-bar__yAxis-linebox">
				...
				>
					<text class="zx-xAxis__val" :style="{ color: item.color || xAxisLabel.color, fontSize: (xAxisLabel.size || 24) + 'rpx' }" v-if="xAxisLabel.show">
						{{ item.value }}
					</text>
				</view>
			</view>
...
const getLeft = (index) => {
	...
	return (((props.isStack ? total : Math.max(...arr)) - props.min) / (maxValue.value - props.min)) * props.width + 'rpx';
};
