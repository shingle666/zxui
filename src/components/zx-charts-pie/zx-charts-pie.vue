<template>
  <view
    class="zx-charts__pie-wrap"
    :class="{ 'zx-charts__pie-vertical': legend.direction !== 'vertical' }"
  >
    <view
      class="zx-pie__legend"
      :class="{ 'zx-legend__flex-column': legend.direction === 'vertical' }"
      v-if="legend.show"
    >
      <view
        class="zx-pie__legend-item"
    ...
    >
      <view
        ...
        :style="{
          ...
          clip:
            item.transformAngle > 180 ? `rect(0, ${diam / 2}rpx, ${diam}rpx, 0)` : 'auto',
        }"
        v-for="(item, index) in dataset"
        :key="index"
      ...
      <view
        ...
        v-if="type == 2"
        :style="{
          width: annular.width + 'rpx',
          height: annular.width + 'rpx',
          backgroundColor: annular.backgroundColor,
        }"
      ></view>
    </view>
    <view
      class="zx-pie__tooltip"
      v-if="tooltip"
      :class="{ 'zx-pie__tooltip-show': tooltipShow }"
    >
      ...
      <view class="zx-pie__tooltip-item" :style="{ paddingTop: title ? '24rpx' : '0' }">
        <view
          ...
          :style="{ backgroundColor: dataset[activeIndex] && dataset[activeIndex].color }"
        ></view>
...
const props = defineProps({
  ...
  legend: {
    type: Object,
    default: () => ({ show: true, size: 24, color: "#333", direction: "horizontal" }),
  },
