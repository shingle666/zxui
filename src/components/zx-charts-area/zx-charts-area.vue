<template>
  <view class="zx-charts__line-wrap" :style="{ width: width + 'rpx' }">
    ...
    <view
      class="zx-charts__line-box"
      v-if="xAxis.length > 0 && dataset.length > 0"
      :style="{ width: width + 'rpx' }"
    >
      <scroll-view
        :scroll-x="scrollable"
        class="zx-line__scroll-view"
        :style="{ height: scrollViewH + 'rpx' }"
      >
        ...
        <view class="zx-charts__line" :style="{ height: height + 'rpx' }">
          ...
          <view
            class="zx-area__chart-box"
            v-for="(area, i) in dots"
            :key="area.key"
            :style="{
              left: xGap / 2 + 'rpx',
              right: xGap / 2 + 'rpx',
          ...
          >
            <view
              class="zx-area__item"
              :style="{
                '--zx-area-start': model.start,
                '--zx-area-end': model.end,
                background: model.background,
              }"
              v-for="(model, j) in area.area"
              :key="j"
            ></view>
          </view>
        </view>
    ...
    <view
      class="zx-line__tooltip"
      v-if="tooltip"
      :class="{ 'zx-line__tooltip-show': tooltipShow }"
    >
...
function getCoordinatePoint() {
  ...
  xWidth.value = itemGap * (xSections - 1);
  xGap.value = itemGap;
  let yHeight = (props.yAxisLine.itemGap || 60) * ySections;
  dataset.value.map((item, index) => {
    ...
    source.map((val, idx) => {
      let y = getValue(
        ((val - props.min) / (maxValue.value - props.min)) * yHeight - radius
      );
      dotArr.push({
        id: "d" + idx,
      ...
      if (idx !== xSections - 1) {
        let y1 = getValue(
          ((source[idx + 1] - props.min) / (maxValue.value - props.min)) * yHeight -
            radius
        );
        areaArr.push({
          start: (y + 6) / yHeight,
...
.zx-area__item {
  ...
  clip-path: polygon(
    0% calc(100% * (1 - var(--zx-area-start))),
