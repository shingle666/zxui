<template>
	<view class="zx-combox" :class="border ? '' : 'zx-combox__no-border'">
		...
		<view class="zx-combox__input-box">
			<input class="zx-combox__input" type="text" :placeholder="placeholder"
				placeholder-class="zx-combox__input-plac" v-model="inputVal" @input="onInput" @focus="onFocus" @blur="onBlur" />
		...
		<view class="zx-combox__selector" v-if="showSelector">
			...
			<scroll-view scroll-y="true" class="zx-combox__selector-scroll" @scroll="onScroll">
				...
				<view class="zx-combox__selector-item" v-for="(item,index) in filterCandidates" :key="index" @click="onSelectorClick(index)">
					<text>{{item}}</text>
				</view>
			</scroll-view>
		</view>
