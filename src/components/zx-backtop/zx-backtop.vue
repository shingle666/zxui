<template>
	<view v-if="isShow" :class="['zx-backtop', showAnimation]" hover-class="zx-hover" :style="[contentStyle]" @click.stop="handleClick">
		<slot>
			<text class="backtop-icon" :class="iconName" :style="{ fontSize: iconSize, color: iconColor }"></text>
		</slot>
	</view>
</template>
 ...
 * @property {String}			target  		触发滚动的对象，暂不支持，预留参数
 ...
 * @example <zx-backtop :scrollTop="scrollTop" @click="handleBacktopClick"></zx-backtop>
 */
...
.backtop-icon {
	font-family: 'backtop-icon' !important;
	font-size: 16px;
	font-style: normal;
