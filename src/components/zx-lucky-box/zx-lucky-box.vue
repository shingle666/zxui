<template>
  <view v-if="isShow" class="lucky-box" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }">
    ...
    ></image>
    <!-- #ifdef APP-PLUS -->
    <view class="lucky-box-btn" @click="toPlay" :style="{ width: btnWidth + 'px', height: btnHeight + 'px' }"></view>
    <!-- #endif -->
    <!-- #ifndef APP-PLUS -->
    <cover-view class="lucky-box-btn" @click="toPlay" :style="{ width: btnWidth + 'px', height: btnHeight + 'px' }"></cover-view>
    <!-- #endif -->
    <!-- #ifndef H5 -->
    <view v-if="myLucky">
...
function getLuckyImage() {
  return getImage.call(this, 'lucky-box', myCanvas)
}
...
function draw() {
  uni.createSelectorQuery().in(this).select('#lucky-box').fields({
    node: true, size: true
  }).exec((res) => {
