<template>
  <view class="zx-dollmachine" :style="{
    width: width,
    height: height
  }">
    <!-- 娃娃机主体 -->
    <view class="machine-box">
      <!-- 抓取工具 -->
      <view class="machine-tools" :style="{
        left: clawPosition.x + 'px',
        top: clawPosition.y + 'px'
      }">
        <!-- 绳子 -->
        <view class="machine-rope" :style="{
          height: ropeHeight + 'px'
        }"></view>

        <!-- 爪子 -->
        <view class="machine-claw">
          <image :src="currentClawImg" mode="aspectFit" class="claw-img" />

          <!-- 抓到的奖品 -->
          <view v-if="grabbedPrize" class="gift-prize">
            <image :src="grabbedPrize.prizeImg" mode="aspectFit" class="prize-img" />
          </view>
        </view>
      </view>
      ...
      <view class="machine-gift-box">
        <view class="box-glass">
          <view v-for="(item, index) in displayPrizes" :key="index" class="gift-img" :style="{
            left: item.x + 'px',
            top: item.y + 'px'
          }">
      ...
      <view class="machine-operate">
        ...
        <view class="machine-direction">
          <view class="direction-block" @touchstart="startMove('left')" @touchend="stopMove"></view>
          <view class="direction-block" @touchstart="startMove('right')" @touchend="stopMove"></view>
        </view>

        <!-- 抓取按钮 -->
        <view class="machine-btn" :class="{
          'machine-start': !isGrabbing && !isMoving,
          'machine-disabled': isGrabbing || isMoving
        }" @click="startGrab"></view>
