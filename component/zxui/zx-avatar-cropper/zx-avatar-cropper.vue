<template>
	<view class="zx-avatar-cropper">
		<!-- 裁剪容器 -->
		<view v-if="showCropper" class="zx-avatar-cropper__container" :style="containerStyle">
			<!-- 图片显示区域 -->
			<view class="zx-avatar-cropper__image-container" :style="imageContainerStyle">
				<image class="zx-avatar-cropper__image" :src="imageSrc" mode="aspectFit" :style="imageStyle"
					@load="onImageLoad" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd">
				</image>
			</view>

			<!-- 裁剪框 -->
			<view class="zx-avatar-cropper__crop-box" :class="[`zx-avatar-cropper__crop-box--${shape}`]"
				:style="cropBoxStyle">
				<!-- 裁剪框边框 -->
				<view class="zx-avatar-cropper__crop-border"></view>

				<!-- 四个角的控制点 -->
				<view v-if="resizable" class="zx-avatar-cropper__handle zx-avatar-cropper__handle--nw"
					@touchstart.stop="onHandleTouchStart('nw', $event)" @touchmove.stop="onHandleTouchMove"
					@touchend.stop="onHandleTouchEnd"></view>
				<view v-if="resizable" class="zx-avatar-cropper__handle zx-avatar-cropper__handle--ne"
					@touchstart.stop="onHandleTouchStart('ne', $event)" @touchmove.stop="onHandleTouchMove"
					@touchend.stop="onHandleTouchEnd"></view>
				<view v-if="resizable" class="zx-avatar-cropper__handle zx-avatar-cropper__handle--sw"
					@touchstart.stop="onHandleTouchStart('sw', $event)" @touchmove.stop="onHandleTouchMove"
					@touchend.stop="onHandleTouchEnd"></view>
				<view v-if="resizable" class="zx-avatar-cropper__handle zx-avatar-cropper__handle--se"
					@touchstart.stop="onHandleTouchStart('se', $event)" @touchmove.stop="onHandleTouchMove"
					@touchend.stop="onHandleTouchEnd"></view>
			</view>
			...
			<view class="zx-avatar-cropper__mask">
				<!-- 上遮罩 -->
				<view class="zx-avatar-cropper__mask-part zx-avatar-cropper__mask-top" :style="maskTopStyle"></view>
				<!-- 左遮罩 -->
				<view class="zx-avatar-cropper__mask-part zx-avatar-cropper__mask-left" :style="maskLeftStyle"></view>
				<!-- 右遮罩 -->
				<view class="zx-avatar-cropper__mask-part zx-avatar-cropper__mask-right" :style="maskRightStyle"></view>
				<!-- 下遮罩 -->
				<view class="zx-avatar-cropper__mask-part zx-avatar-cropper__mask-bottom" :style="maskBottomStyle">
				</view>
			</view>
		</view>
		...
		<view v-if="showCropper" class="zx-avatar-cropper__toolbar">
			<view class="zx-avatar-cropper__toolbar-left">
				<zx-button v-if="showRotateBtn" type="default" size="small" @click="rotateLeft">
					<zx-icon name="rotate-left" size="32rpx"></zx-icon>
				</zx-button>
				<zx-button v-if="showRotateBtn" type="default" size="small" @click="rotateRight"
					style="margin-left: 20rpx;">
					<zx-icon name="rotate-right" size="32rpx"></zx-icon>
				</zx-button>
				<zx-button v-if="showResetBtn" type="default" size="small" @click="reset" style="margin-left: 20rpx;">
					重置
				</zx-button>
			</view>
			<view class="zx-avatar-cropper__toolbar-right">
				<zx-button type="default" size="small" @click="cancel">
					取消
				</zx-button>
				<zx-button type="primary" size="small" @click="confirm" style="margin-left: 20rpx;">
					确定
				</zx-button>
			</view>
		</view>
		...
		<view v-if="!showCropper" class="zx-avatar-cropper__select">
			<zx-button type="primary" @click="chooseImage" :loading="loading">
				{{ selectText }}
			</zx-button>
		</view>
	</view>
