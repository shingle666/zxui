<template>
	<view class="zx-cell" :style="[customStyle]" :hover-class="!disabled && (clickable || isLink) ? 'zx-cell--clickable' : ''" :hover-stay-time="250" @tap="clickHandler">
			...
			<view class="zx-cell__body__content">
				...
				<view class="zx-cell__title">
					<slot name="title">
						<text
							v-if="title"
							class="zx-cell__title-text"
							:style="[titleTextStyle]"
							:class="[disabled && 'zx-cell--disabled', size === 'large' && 'zx-cell__title-text--large']"
						>
							{{ title }}
						</text>
					</slot>
					<slot name="label">
						<text class="zx-cell__label" v-if="label" :class="[disabled && 'zx-cell--disabled', size === 'large' && 'zx-cell__label--large']">{{ label }}</text>
					</slot>
				</view>
			</view>
			<slot name="value">
				<text class="zx-cell__value" :class="[disabled && 'zx-cell--disabled', size === 'large' && 'zx-cell__value--large']">{{ value }}</text>
			</slot>
			<view class="zx-cell__right-icon-wrap" v-if="$slots['right-icon'] || isLink" :class="[`zx-cell__right-icon-wrap--${arrowDirection}`]">
...
// 计算属性
const titleTextStyle = computed(() => props.titleStyle);
