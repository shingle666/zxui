<template>
  <view class="zx-carousel-item" :class="[
    type === 'card' && 'zx-carousel-item--card',
    direction === 'vertical' && 'zx-carousel-item--vertical',
  ]" :style="itemStyle" @tap="handleClick">
    <view class="zx-carousel-item__wrapper" :class="[type === 'card' && 'zx-carousel-item__wrapper--card']"
      :style="wrapperStyle">
      <!-- 内容插槽 -->
      <slot>
        <!-- 默认图片展示 -->
        <image v-if="src && getItemType() === 'image'" class="zx-carousel-item__image" :src="src" :mode="imageMode"
          :lazy-load="lazyLoad" @load="handleImageLoad" @error="handleImageError" />
        <!-- 默认内容展示 -->
