<template>
  <view class="zx-carousel" :style="carouselStyle">
    <!-- 轮播容器 -->
    <swiper class="zx-carousel__container" :style="containerStyle" :circular="circular"
      :interval="autoplay ? interval : 0" :duration="duration" :current="currentIndex" :current-item-id="currentItemId"
      :previous-margin="previousMargin" :next-margin="nextMargin" :display-multiple-items="displayMultipleItems"
      :vertical="direction === 'vertical'" :disable-touch="disableTouch" :touchable="touchable" :indicator-dots="false"
      :autoplay="autoplay" :easing-function="easingFunction" @change="handleSwiperChange" @transition="handleTransition"
      @animationfinish="handleAnimationFinish" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
      <swiper-item v-for="(item, index) in list" :key="getItemKey(item, index)" :item-id="getItemId(item, index)"
        class="zx-carousel__item" :class="[
          type === 'card' && 'zx-carousel__item--card',
          direction === 'vertical' && 'zx-carousel__item--vertical',
        ]" :style="getItemStyle(index)">
        <view class="zx-carousel__item-wrapper" :class="[
        ...
        ]" :style="getItemWrapperStyle(index)" @tap="handleItemClick(index)">
          <!-- 内容插槽 -->
          <slot name="item" :item="item" :index="index">
            <!-- 默认图片展示 -->
            <image v-if="getItemType(item) === 'image'" class="zx-carousel__item-image" :src="getItemSrc(item)"
              :mode="imageMode" :lazy-load="lazyLoad" @load="handleImageLoad" @error="handleImageError" />
            <!-- 默认内容展示 -->
    ...
    <!-- 指示器 -->
    <view v-if="showIndicator && indicatorPosition !== 'none'" class="zx-carousel__indicators" :class="[
      ...
      <slot name="indicator" :current="currentIndex" :total="list.length">
        <view v-for="(item, index) in list" :key="`indicator-${index}`" class="zx-carousel__indicator" :class="[
          index === currentIndex && 'zx-carousel__indicator--active',
          `zx-carousel__indicator--${trigger}`,
        ]" :style="getIndicatorStyle(index)" @tap="handleIndicatorClick(index)"
          @mouseenter="handleIndicatorHover(index)">
          <text v-if="item.label" class="zx-carousel__indicator-label">{{
            item.label
          }}</text>
        </view>
      </slot>
    ...
    <!-- 切换箭头 -->
    <view v-if="showArrow && arrow !== 'never'" class="zx-carousel__arrow zx-carousel__arrow--left" :class="[
      arrow === 'always' && 'zx-carousel__arrow--always',
      direction === 'vertical' && 'zx-carousel__arrow--vertical',
    ]" :style="arrowStyle" @tap="handlePrev">
      <slot name="arrow-left">
        <zx-icon name="chevron-left" size="32rpx" color="#fff"></zx-icon>
      </slot>
    </view>
    <view v-if="showArrow && arrow !== 'never'" class="zx-carousel__arrow zx-carousel__arrow--right" :class="[
      arrow === 'always' && 'zx-carousel__arrow--always',
      direction === 'vertical' && 'zx-carousel__arrow--vertical',
    ]" :style="arrowStyle" @tap="handleNext">
      <slot name="arrow-right">
        <zx-icon name="chevron-right" size="32rpx" color="#fff"></zx-icon>
      </slot>
    </view>
  </view>
</template>
...
function setActiveItem(index) {
  if (typeof index === "string") {
    // 通过 name 查找
    const targetIndex = props.list.findIndex(
      (item) => typeof item === "object" && item !== null && item.name === index
    );
    if (targetIndex !== -1) {
