<template>
	<view class="zx-avatar-group">
		<view class="zx-avatar-group__item" v-for="(item, index) in showUrl" :key="index"
			:style="{ marginLeft: index === 0 ? 0 : -size * gap + 'rpx' }">
			<zx-avatar :size="size + 'rpx'" :shape="shape" :mode="mode"
				:src="testObject(item) ? (keyName && item[keyName]) || item.url : item"></zx-avatar>
			<view class="zx-avatar-group__item__show-more"
				v-if="showMore && index === showUrl.length - 1 && (urls.length > maxCount || extraValue > 0)"
				@tap="clickHandler" :style="{ borderRadius: shape === 'circle' ? '100px' : '8rpx' }">
				<zx-text color="#ffffff" :size="size * 0.4 + 'rpx'"
					:text="`+${extraValue || urls.length - showUrl.length}`" align="center"
