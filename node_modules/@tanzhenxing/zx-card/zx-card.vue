<template>
  <view class="zx-card" :class="{
    ...
    }" @click="onClick('card')" :hover-class="clickable ? 'zx-card--hover' : ''">
    <!-- 封面 -->
    <slot name="cover">
      <view v-if="cover" class="zx-card__cover" :style="{ 'border-radius': coverRadius + 'rpx' }">
        <image class="zx-card__cover-image" :mode="coverMode" @click.stop="onClick('cover')" :src="cover"
      ...
      <view v-if="title || extra" class="zx-card__header">
        ...
        <view class="zx-card__header-box" @click.stop="onClick('title')">
          <view v-if="thumbnail" class="zx-card__header-avatar" :style="{
            ...
            <image class="zx-card__header-avatar-image" :src="thumbnail" :mode="avatarMode" :lazy-load="lazyLoad" />
          </view>
          <view class="zx-card__header-content">
            ...
            <text v-if="title && subTitle" class="zx-card__header-content-subtitle zx-ellipsis" :style="{
        ...
        <view class="zx-card__header-extra" @click.stop="onClick('extra')">
          <slot name="extra">
            <text class="zx-card__header-extra-text" :style="{ color: extraColor }">{{
              extra
              }}</text>
          </slot>
        </view>
    ...
    <!-- 卡片内容 -->
    <view class="zx-card__content" :style="{
      padding: padding,
...
const props = defineProps({
  ...
  mode: {
    ...
    default: "default",
    validator: (value) => {
      return ["default", "primary", "success", "warning", "error", "info"].includes(
        value
      );
    },
  },
...
$zx-shadow-base: 0 0px 6px 1px rgba($color: #a5a5a5,
    $alpha: 0.2,
  ) !default;
