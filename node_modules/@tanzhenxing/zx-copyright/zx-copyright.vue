<template>
  <view class="zx-copyright" :style="{ backgroundColor: backgroundColor, padding: padding, textAlign: align }"
    @click="handleClick">
...
const props = defineProps({
  ...
  text: {
    type: String,
    default: "© 2025 YourCompany. All rights reserved.",
  },
  // 背景颜色
  backgroundColor: {
    type: String,
...
const handleClick = () => {
  if (props.link) {
    ...
    // #ifndef H5
    uni.navigateTo({
      url: `${props.webviewPath}?url=${encodeURIComponent(props.link)}`, // 假设有一个webview页面用于打开链接
    });
    // #endif
  }
};
