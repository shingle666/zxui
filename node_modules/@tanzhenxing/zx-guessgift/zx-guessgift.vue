  ...
  <view class="zx-guessgift" :style="{
    ...
    <view class="guess-gift-container">
      ...
      <view class="bowl-container">
        <view v-for="(bowl, index) in bowlList" :key="index" class="bowl-item" :class="{
          ...
          <!-- 碗的图片 -->
          <image :src="bowlImg" mode="aspectFit" class="bowl-image" />

          <!-- 奖品（隐藏在碗下） -->
          <view class="gold-bean" :class="{
            '_opacity0': !bowl.showPrize,
            '_opacity1': bowl.showPrize
          }">
            <image :src="prizeImg" mode="aspectFit" class="prize-image" />
          </view>
        </view>
      </view>

      <!-- 开始按钮 -->
      <view class="start-button" :class="{
...
// 定义 emits
const emit = defineEmits(['start-turns', 'end-turns', 'bowl-click'])

// 获取当前实例
const instance = getCurrentInstance()

// 响应式数据
const bowlList = reactive([
  {
    id: 0,
    translateX: 0,
    translateY: 0,
...
const initGame = () => {
  ...
  // 设置奖品位置
  currentPrizeIndex.value = props.prizeIndex >= 0 ? props.prizeIndex : Math.floor(Math.random() * 3)
...
// 开始交换动画
const startSwapping = () => {
  if (currentTurns.value >= props.turnsNumber) {
    endSwapping()
    return
  }
  ...
  swapBowls(bowl1Index, bowl2Index)
...
// 交换两个碗
const swapBowls = (index1, index2) => {
  const bowl1 = bowlList[index1]
  const bowl2 = bowlList[index2]
  ...
  bowl1.isRaised = true
  bowl2.isRaised = true
  bowl1.translateY = -props.raiseHeight
  bowl2.translateY = -props.raiseHeight
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // 更新奖品位置
        if (currentPrizeIndex.value === index1) {
          currentPrizeIndex.value = index2
        } else if (currentPrizeIndex.value === index2) {
          currentPrizeIndex.value = index1
        }
      }, 200)
    }, 300)
  }, 200)
...
// 点击碗
const onBowlClick = (index) => {
  if (isAnimating.value) return

  // 显示选中碗下的奖品
  bowlList[index].showPrize = (index === currentPrizeIndex.value)

  // 抬起碗
  bowlList[index].isRaised = true
  bowlList[index].translateY = -props.raiseHeight

  // 触发点击事件
  emit('bowl-click', {
    index,
    isWin: index === currentPrizeIndex.value,
