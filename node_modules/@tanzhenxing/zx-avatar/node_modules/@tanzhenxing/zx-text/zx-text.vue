<template>
	<view class="zx-text" :class="[block ? 'block' : 'inline', mode === 'link' ? 'link-text' : '']"
		:style="justifyStyle" @click="clickHandler">
		<slot name="left"></slot>
		<text :class="[mode === 'price' ? 'price' : 'value', lines && `zx-line-${lines}`]" :style="[valueStyle,customStyle]"
			:selectable="selectable" :userSelect="userSelect" :space="space" :decode="decode">
...
const clickHandler = () => {
	...
	if (props.mode === 'link' && props.href) {
		if (props.target === '_blank') {
			...
			// #ifdef MP
			uni.navigateTo({
				url: `${props.webviewPath}?url=${encodeURIComponent(props.href)}`
			});
			// #endif
		} else {
			// 打开内部链接
