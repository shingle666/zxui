<template>
  <view class="zx-lazyload" :style="[viewStyle]">
    <image
      v-if="isLoad"
      :class="['zx-lazyload__image', { 'zx-lazyload__image--fade-in': showImage && fadeIn }]"
      :src="currentSrc"
      :mode="mode"
      :webp="webp"
      :lazy-load="false" 
    ...
    />
    <view
      v-if="!isLoad && (loadingSrc || useLoadingSlot)"
      class="zx-lazyload__loading"
      :style="[imageStyle, imageCustomStyle]"
    >
      <slot v-if="useLoadingSlot" name="loading"></slot>
      <image
        v-else-if="loadingSrc"
        :src="loadingSrc"
        :mode="mode"
        :webp="webp"
        class="zx-lazyload__loading-img"
        :style="[imageStyle, imageCustomStyle]"
      />
    </view>
    <view
      v-if="isError && !isLoad && (errorSrc || useErrorSlot)"
      class="zx-lazyload__error"
      :style="[imageStyle, imageCustomStyle]"
    >
