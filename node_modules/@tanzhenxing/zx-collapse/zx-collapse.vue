<template>
  <view class="zx-collapse" :class="{ 'zx-collapse--border': border }">
    <view v-for="(item, index) in items" :key="item.name || index" class="zx-collapse-item" :class="{
      ...
      <!-- 折叠面板头部 -->
      <view class="zx-collapse-item__header" :class="{
        ...
        <!-- 图标 -->
        <view class="zx-collapse-item__arrow" :class="{
          ...
          <slot name="icon" :item="item" :isActive="isActive(item.name || index)">
            <zx-icon :name="item.icon || 'arrow-right'" :size="iconSize" :color="item.disabled
                ? disabledColor
                : isActive(item.name || index)
                  ? activeColor
                  : normalColor
              "></zx-icon>
          </slot>
        ...
        <!-- 标题 -->
        <view class="zx-collapse-item__title" :style="{
      ...
      <!-- 折叠面板内容 -->
      <view class="zx-collapse-item__wrap" :class="{ 'zx-collapse-item__wrap--active': isActive(item.name || index) }"
        :style="{
          height: isActive(item.name || index) ? 'auto' : '0',
          overflow: 'hidden',
          transition: `all ${animationDuration}ms ease-in-out`,
        }">
        <view class="zx-collapse-item__content" :style="{
