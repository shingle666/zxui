<template>
  <view v-if="modelValue" class="zx-dialog-container" :class="{ 'zx-dialog-center': center || alignCenter }"
    :style="containerStyle" @click.stop="handleWrapperClick">
    <view ref="dialogRef" class="zx-dialog" :class="[
      customClass,
      ...
      <!-- 可拖拽处理 -->
      <view v-if="draggable" class="zx-dialog-header-draggable" @touchstart="handleDragStart"
        @touchmove="handleDragMove" @touchend="handleDragEnd"></view>

      <!-- 对话框头部 -->
      <slot name="header">
        <view class="zx-dialog-header" :class="headerClass">
          <text v-if="title" class="zx-dialog-title" :id="titleId">{{ title }}</text>
          <view v-if="showClose" class="zx-dialog-close" @click="handleClose">
