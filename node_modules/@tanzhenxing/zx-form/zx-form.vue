<template>
	<form class="zx-form" :class="[
		labelPosition ? `zx-form--label-${labelPosition}` : '',
		{'zx-form--inline': inline},
		{[`zx-form--${size}`]: size}
	]" :disabled="disabled" :report-submit="reportSubmit" @submit="handleSubmit" @reset="handleReset">
		<slot></slot>
	</form>
</template>
...
<script setup>
	...
	function validateField(fieldProps, callback) {
		...
		const propsArray = [].concat(fieldProps);
		const validateFields = fields.filter(field => propsArray.indexOf(field.prop) !== -1);
	...
	function clearValidate(fieldProps = []) {
		...
		const fieldsToValidate = fieldProps.length
