<template>
  <view class="zx-album">
    <view class="zx-album__row" ref="zx-album__row" v-for="(arr, index) in showUrls" :forComputedUse="albumWidth"
      :key="index">
      <view class="zx-album__row__wrapper" v-for="(item, index1) in arr" :key="index1"
        ...
        " class="zx-album__row__wrapper__text">
          <zx-text :text="`+${urls.length - maxCount}`" color="#ffffff" size="70rpx" align="center"
            customStyle="justify-content: center"></zx-text>
        </view>
...
const albumWidth = computed(() => {
  ...
  if (props.urls.length === 1) {
    ...
    width =
      rowItemCount > 0
        ? rowItemCount * props.multipleSize + props.space * (rowItemCount - 1)
        : 0;
  }
