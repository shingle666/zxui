<template>
  <view class="zx-contact-edit" :style="[customStyle]">
    <zx-form class="zx-contact-edit__form" @submit="handleSubmit">
      <!-- 表单字段区域 -->
      <view class="zx-contact-edit__fields">
        ...
        <view class="zx-contact-edit__field">
          ...
          <view class="zx-contact-edit__input-wrapper">
            <zx-input class="zx-contact-edit__input" v-model="contactData.name" type="text"
              :placeholder="namePlaceholder || '请输入姓名'" :maxlength="30" :disabled="disabled" @input="onNameInput"
              @blur="validateName" />
            <view class="zx-contact-edit__clear" v-if="clearable && contactData.name && !disabled" @click="clearName">
              <zx-icon name="close" size="16" color="#999" class="zx-contact-edit__clear-icon"></zx-icon>
            </view>
          </view>
        </view>
        ...
        <view class="zx-contact-edit__field">
          ...
          <view class="zx-contact-edit__input-wrapper">
            <zx-input class="zx-contact-edit__input" v-model="contactData.tel" type="number"
              :placeholder="telPlaceholder || '请输入手机号'" :maxlength="11" :disabled="disabled" @input="onTelInput"
              @blur="validateTel" />
            <view class="zx-contact-edit__clear" v-if="clearable && contactData.tel && !disabled" @click="clearTel">
              <zx-icon name="close" size="16" color="#999" class="zx-contact-edit__clear-icon"></zx-icon>
            </view>
          </view>
        </view>
      ...
      <view class="zx-contact-edit__switch-cell" v-if="showSetDefault">
        ...
        <view class="zx-contact-edit__switch-wrapper">
          <zx-switch :checked="contactData.isDefault" :disabled="disabled" @change="onDefaultChange" color="#07c160" />
        </view>
      </view>

      <!-- 错误提示 -->
      <view class="zx-contact-edit__errors" v-if="errors.length > 0">
        <view class="zx-contact-edit__error" v-for="(error, index) in errors" :key="index">
      ...
      <view class="zx-contact-edit__buttons">
        <!-- 保存按钮 -->
        <zx-button class="zx-contact-edit__button zx-contact-edit__button--save"
          :class="{ 'zx-contact-edit__button--loading': isSaving }" :disabled="disabled || isSaving" :loading="isSaving"
          form-type="submit" type="primary">
          {{ saveText || '保存' }}
        </zx-button>

        <!-- 删除按钮 -->
        <zx-button class="zx-contact-edit__button zx-contact-edit__button--delete"
          :class="{ 'zx-contact-edit__button--loading': isDeleting }" :disabled="disabled || isDeleting"
          :loading="isDeleting" v-if="isEdit" @click="handleDelete" type="danger">
          {{ deleteText || '删除' }}
        </zx-button>
      </view>
    </zx-form>
