<template>
  ...
  >
    <!-- 触发器内容 -->
    <view 
      class="zx-ai-suggestion__trigger"
        ...
        <template v-for="(item, index) in flattenItems" :key="item.value">
          ...
          >
            ...
            <view v-if="item.extra" class="zx-ai-suggestion__item-extra">
              <component :is="item.extra" v-if="typeof item.extra === 'object'" />
              <text v-else>{{ item.extra }}</text>
            </view>
