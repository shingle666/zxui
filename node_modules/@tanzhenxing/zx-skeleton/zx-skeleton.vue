<template>
	<view class="zx-skeleton" :class="{ 'zx-skeleton--animated': animated }">
		...
		<view v-if="internalLoading" class="zx-skeleton__wrapper">
			<!-- 渲染多个模板 -->
			<view v-for="(item, index) in count" :key="index" class="zx-skeleton__item"
				...
				<slot name="template">
					...
					<view class="zx-skeleton__paragraph">
						<zx-skeleton-item v-for="row in rows" :key="row" variant="text"
							:width="row === 1 ? '33%' : row === rows ? '61%' : '100%'"
							:style="{ marginBottom: row < rows ? '16rpx' : '0' }" :animated="animated" />
					</view>
				</slot>
			</view>
		</view>
