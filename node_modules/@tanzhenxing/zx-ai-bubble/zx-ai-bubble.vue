<template>
  <view :class="bubbleClasses" :style="bubbleStyles">
    ...
    <view v-if="showAvatar" :class="avatarClasses" :style="avatarStyles">
      <slot name="avatar">
        <zx-avatar :src="avatar?.src" :text="avatar?.text" :icon="avatar?.icon" :shape="avatar?.shape || 'circle'"
          :size="avatar?.size || '80rpx'" :bg-color="avatar?.bgColor || '#c0c4cc'" :color="avatar?.color || '#ffffff'"
          :font-size="avatar?.fontSize || '32rpx'" :mode="avatar?.mode || 'aspectFill'"
          :custom-class="avatarImageClasses" :style="avatarImageStyles" />
      </slot>
    </view>
