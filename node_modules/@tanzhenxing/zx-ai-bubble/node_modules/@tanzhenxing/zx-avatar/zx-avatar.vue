<template>
	<view class="zx-avatar" :class="[`zx-avatar--${shape}`, loading ? 'zx-avatar--loading' : '', customClass]"
		:style="avatarStyle" @click="clickHandler">
		<slot>
			...
			<image v-else class="zx-avatar__image" :class="[`zx-avatar__image--${shape}`, imgClass]"
				:src="avatarUrl || defaultUrl" :mode="mode" @error="errorHandler" @load="loadCompleted"
				:style="[{ width: size, height: size }]"></image>
			<view v-if="loading && !text && !icon && !mpAvatar" class="zx-avatar__loading">
				<zx-loading />
			</view>
			<zx-badge v-if="badge" :text="badge" :bg-color="badgeColor" :offset="badgeOffset" :is-dot="badgeIsDot"
				:show-zero="badgeShowZero" :absolute="badgePosition" :size="badgeFontSize" :scale="badgeScale"
