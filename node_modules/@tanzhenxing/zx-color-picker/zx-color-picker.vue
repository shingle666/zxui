<template>
  <view class="zx-color-picker" :class="colorPickerClass">
    <view class="zx-color-picker__trigger" :class="{ 'is-disabled': disabled }" @tap="handleTriggerClick"
      :tabindex="disabled ? -1 : tabindex" @keydown.enter="handleTriggerClick" @focus="handleFocus" @blur="handleBlur"
      :aria-label="ariaLabel" :id="id">
      <view class="zx-color-picker__color" v-if="modelValue">
        <view class="zx-color-picker__color-inner" :style="{ backgroundColor: displayedColor }"></view>
      </view>
      <view class="zx-color-picker__empty" v-else>
        <svg class="zx-color-picker__icon" viewBox="0 0 1024 1024">
          <path
            d="M981.184 160.096C837.568 139.456 678.848 128 512 128S186.432 139.456 42.816 160.096C15.296 267.808 0 386.848 0 512s15.296 244.192 42.816 351.904C186.432 884.544 345.152 896 512 896s325.568-11.456 469.184-32.096C1008.704 756.192 1024 637.152 1024 512s-15.296-244.192-42.816-351.904zM384 672l-128-128 128-128 45.248 45.248L364.512 512l64.736 50.752L384 672z m256 0l-45.248-109.248L659.488 512l-64.736-50.752L640 352l128 128-128 192z">
          </path>
        </svg>
      </view>
      <view class="zx-color-picker__icon">
        <svg viewBox="0 0 1024 1024">
          <path
            d="M831.872 340.864L512 652.672 192.128 340.864a30.592 30.592 0 000 42.752L481.664 771.136a42.496 42.496 0 0060.672 0L831.872 383.616a30.592 30.592 0 000-42.752z">
          </path>
        </svg>
      </view>
    </view>
    ...
    <view v-if="showPanel" class="zx-color-picker__panel-container" @tap="handleClickOutside">
      <view class="zx-color-picker__panel" :class="popperClass" :style="panelStyle" @tap.stop>
        <view class="zx-color-picker__panel-inner">
          <!-- 颜色选择区域 -->
          <view class="zx-color-picker__saturation-panel">
            <view class="zx-color-picker__saturation" :style="{ backgroundColor: hueColor }"
              @touchstart.stop.prevent="handleSaturationTouch" @touchmove.stop.prevent="handleSaturationTouch">
              <view class="zx-color-picker__white"></view>
              <view class="zx-color-picker__black"></view>
              <view class="zx-color-picker__cursor" :style="saturationCursorStyle">
                <view></view>
              </view>
            </view>
          </view>
          ...
          <view class="zx-color-picker__controls">
            <view class="zx-color-picker__hue-slider" @touchstart.stop.prevent="handleHueTouch"
              @touchmove.stop.prevent="handleHueTouch">
              <view class="zx-color-picker__hue-cursor" :style="hueCursorStyle"></view>
            </view>

            <!-- 透明度滑块 -->
            <view v-if="showAlpha" class="zx-color-picker__alpha-slider" @touchstart.stop.prevent="handleAlphaTouch"
              @touchmove.stop.prevent="handleAlphaTouch">
              <view class="zx-color-picker__alpha-bar" :style="alphaBarStyle">
                <view class="zx-color-picker__alpha-cursor" :style="alphaCursorStyle"></view>
              </view>
            </view>
          </view>
          ...
          <view v-if="predefine && predefine.length" class="zx-color-picker__predefine">
            <view class="zx-color-picker__predefine-color-list">
              <view v-for="(color, index) in predefine" :key="index" class="zx-color-picker__predefine-color"
                @tap="handlePredefineClick(color)">
                <view class="zx-color-picker__predefine-color-inner" :style="{ backgroundColor: color }"></view>
              </view>
            </view>
          </view>
          ...
          <view class="zx-color-picker__input">
            <zx-input v-model="inputValue" @blur="handleInputBlur" @confirm="handleInputEnter"
              class="zx-color-picker__input-inner" type="text" />
          </view>

          <!-- 按钮区域 -->
          <view class="zx-color-picker__btns">
            <zx-button class="zx-color-picker__btn--clear" @click="handleClear">
              清空
            </zx-button>
            <zx-button type="primary" class="zx-color-picker__btn--confirm" @click="handleConfirm">
              确定
            </zx-button>
          </view>
        </view>
...
const formatColor = (colorObj) => {
  ...
  if (props.colorFormat === "hsl") {
    return props.showAlpha && alpha < 1
      ? `hsla(${hue}, ${saturation}%, ${Math.round(
