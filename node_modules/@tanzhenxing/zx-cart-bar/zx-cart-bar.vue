<template>
	<view :class="['zx-cart-bar', customClass, { 'zx-cart-bar--safe-area': safeAreaInsetBottom }]"
		:style="[customStyle]">
		<!-- 顶部提示信息 -->
		...
		<view class="zx-cart-bar__main">
			...
			<view v-if="showCheckAll" class="zx-cart-bar__check-all" @tap="handleCheckAll">
				<zx-checkbox v-model="isAllChecked" :disabled="disabled || totalCount === 0" :color="checkboxColor"
					@change="onCheckAllChange"></zx-checkbox>
			...
			<view class="zx-cart-bar__price-info">
				...
				<view v-if="showOriginalPrice && originalPrice > 0" class="zx-cart-bar__original-price">
					<text class="zx-cart-bar__original-price-text">原价：{{ currency }}{{ (originalPrice / 100).toFixed(2)
						}}</text>
				</view>
			...
			<view class="zx-cart-bar__button-group">
				...
				<slot name="button">
					<!-- 默认结算按钮 -->
					<zx-button :type="buttonType" :color="buttonColor"
						:disabled="disabled || checkedCount === 0 || loading" :loading="loading" :round="buttonRound"
						class="zx-cart-bar__button" @click="handleSubmit">
						{{ getButtonText() }}
					</zx-button>
				</slot>
			</view>
