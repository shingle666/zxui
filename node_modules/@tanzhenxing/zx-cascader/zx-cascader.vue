<template>
  <view @touchmove.stop.prevent="stop">
    <!-- 遮罩层 -->
    <view class="zx-cascader__mask" :class="{ 'zx-cascader__mask--show': show }" :style="getStyles"
      @tap.stop="maskClose"></view>

    <!-- 级联选择器主体 -->
    <view class="zx-cascader__wrap" :class="{ 'zx-cascader__wrap--show': show }" :style="{
      ...
      <!-- 头部工具栏 -->
      <view class="zx-cascader__header" :style="{
        background: background,
        borderTopLeftRadius: radius + 'rpx',
        borderTopRightRadius: radius + 'rpx',
      }">
        <text class="zx-cascader__header-text" :style="{
        ...
        <view class="zx-cascader__header-line" :style="{ background: dividerColor }"></view>
      </view>
      ...
      <view class="zx-cascader__content" :style="{ height: height + 'rpx' }">
        ...
        <view v-if="showTabs" class="zx-cascader__tabs">
          <view v-for="(tab, index) in activeTabs" :key="index" class="zx-cascader__tab"
            :class="{ 'zx-cascader__tab--active': index === activeTabIndex }"
            :style="{ color: index === activeTabIndex ? activeColor : tabColor }" @tap="switchTab(index)">
            {{ tab }}
          </view>
        </view>

        <!-- 选项列表 -->
        <scroll-view scroll-y class="zx-cascader__scroll" :show-scrollbar="false"
          :style="{ height: (showTabs ? height - 80 : height) + 'rpx' }">
          <view class="zx-cascader__list">
            <view v-for="(item, index) in currentOptions" :key="getItemKey(item, index)" class="zx-cascader__item"
              :class="{
                'zx-cascader__item--active': isItemSelected(item),
                'zx-cascader__item--disabled': item[disabledKey],
              }" :style="{ padding: padding }" @tap="selectItem(item, index)">
              <!-- 选项内容 -->
              <view class="zx-cascader__item-content">
                <!-- 图标 -->
                <view v-if="item[iconKey]" class="zx-cascader__icon"
                  :style="{ width: iconSize + 'rpx', height: iconSize + 'rpx' }">
                  <image :src="item[iconKey]" :style="{ width: iconSize + 'rpx', height: iconSize + 'rpx' }"
                    mode="aspectFit" />
                </view>

                <!-- 文本 -->
                <text class="zx-cascader__item-text" :style="{
                  fontSize: size + 'rpx',
                ...
                <view class="zx-cascader__item-suffix">
                  <!-- 多选模式的复选框 -->
                  <view v-if="multiple && !checkStrictly" class="zx-cascader__checkbox"
                    :class="{ 'zx-cascader__checkbox--checked': item.checked }" :style="{
                      background: item.checked ? checkboxColor : 'transparent',
                      borderColor: item.checked ? checkboxColor : borderColor,
                    }">
                    <view v-if="item.checked" class="zx-cascader__checkmark" :style="{
                  ...
                  <view v-else-if="hasChildren(item)" class="zx-cascader__arrow">
                    <zx-icon name="chevron-right" color="#ccc" size="24rpx"></zx-icon>
                  </view>

                  <!-- 单选模式的选中标记 -->
                  <view v-else-if="!multiple && isItemSelected(item)" class="zx-cascader__check">
              ...
              <!-- 分割线 -->
              <view v-if="dividerLine && index < currentOptions.length - 1" class="zx-cascader__item-line"
                :style="{ background: dividerColor }"></view>
            </view>

            <!-- 空状态 -->
            <view v-if="currentOptions.length === 0" class="zx-cascader__empty">
              <text class="zx-cascader__empty-text" :style="{ color: '#999', fontSize: '28rpx' }">
                {{ emptyText }}
              </text>
            </view>
          </view>
      ...
      <view class="zx-cascader__footer">
        <view class="zx-cascader__btn-group">
          <view class="zx-cascader__btn zx-cascader__btn--cancel" :style="{ color: cancelColor }"
            @click.stop="handleCancel">
            {{ cancelText }}
          </view>
          <view class="zx-cascader__btn zx-cascader__btn--confirm"
            :style="{ background: confirmBackground, color: confirmColor }" @tap.stop="handleConfirm">
            {{ confirmText }}
          </view>
        </view>
      </view>
