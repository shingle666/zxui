<template>
  <view class="zx-autocomplete">
    <view class="zx-autocomplete__wrapper">
      <zx-input v-model="currentValue" class="zx-autocomplete__input" :placeholder="placeholder" :disabled="disabled"
        :class="{ 'is-disabled': disabled }" @input="onInput" @focus="onFocus" @blur="onBlur" :focus="isFocus" />
      <view class="zx-autocomplete__suffix" v-if="clearable && currentValue" @click="onClear">
        <zx-icon name="close" />
      </view>
      <slot name="suffix"></slot>
    </view>

    <view v-show="suggestionVisible" class="zx-autocomplete__suggestion" :class="{ 'is-loading': loading }"
      ...
      <scroll-view v-else scroll-y class="zx-autocomplete__list">
        ...
        <view v-else v-for="(item, index) in suggestions" :key="index" class="zx-autocomplete__item"
          :class="{ 'is-active': highlightedIndex === index }" @tap="() => onSelect(item)">
          <slot :item="item">
