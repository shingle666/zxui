<template>
  <view class="zx-searchbar">
    <view class="zx-searchbar__box" :style="{ borderRadius: radius + 'px', backgroundColor: bgColor }"
      @click="onBoxClick">
      ...
      <zx-input v-if="showInput || modelValue" class="zx-searchbar__input" :focus="focusSync" :disabled="readonly"
        :placeholder="placeholderText" :maxlength="maxlength" confirm-type="search" type="text" v-model="inputValue"
        :style="{ color: textColor }" @confirm="onConfirm" @blur="onBlur" @focus="onFocus" @input="onInput" />
      <text v-else class="zx-searchbar__placeholder">{{ placeholder }}</text>
      <view v-if="showInput && (clearButton === 'always' || (clearButton === 'auto' && inputValue !== '')) && !readonly"
        class="zx-searchbar__icon-clear" @click.stop="onClear">
        <slot name="clearIcon">
          <zx-icon color="#c0c4cc" size="20" name="clear" />
        </slot>
      </view>
    </view>
    <text v-if="cancelButton === 'always' || (showInput && cancelButton === 'auto')" class="zx-searchbar__cancel"
      @click="onCancel">{{ cancelTextI18n }}</text>
  </view>
</template>
