<template>
	<view class="zx-select" @click.stop>
		...
		<view class="zx-select__header">
			...
			>
				...
				<view class="zx-select__input-box" v-if="!multiple">
					<view v-if="modelValue && selected" class="zx-select__input-text" :style="{ color: showOptions ? activeColor : '' }">{{ getSelectedLabel }}</view>
					...
					<view v-if="modelValue && modelValue.length > 0" class="zx-select__tags-wrap">
						...
						>
							<slot name="tag" :item="item">
								...
								<text class="zx-select__tag-close" @click.stop="removeTag(item)">×</text>
							</slot>
						</view>
					</view>
					<view v-else class="zx-select__input-text zx-select__input-placeholder">{{ placeholder }}</view>
				...
				<view class="zx-select__clear" v-if="clear && modelValue && !disabled" @click.stop="clearSelection">
					<text class="zx-select__clear-icon">×</text>
				</view>
				
				<!-- 箭头图标 -->
				<view class="zx-select__arrow">
					<text class="zx-select__arrow-icon" :class="{'zx-select__arrow-icon--reverse': showOptions}">▼</text>
				</view>
			</view>
		</view>
		...
		<view 
			...
			:class="{'zx-select__dropdown--top': placement === 'top'}"
		...
		>
			...
			<view class="zx-select__search-wrap" v-if="filterable">
				<input 
					class="zx-select__search"
					v-model="query"
					:placeholder="searchPlaceholder"
					confirm-type="search"
					@input="handleQueryChange"
					@confirm="handleQueryConfirm"
				/>
			</view>
			
			<!-- 选项列表 -->
			<scroll-view 
				class="zx-select__dropdown-list" 
				scroll-y 
					...
					<block v-for="(group, groupIndex) in groupedOptions" :key="groupIndex">
						...
						>
							<slot name="option" :item="item">
								...
								<view v-else-if="multiple" class="zx-select__checkbox" :class="{'zx-select__checkbox--checked': isSelected(item)}">
			...
			<view v-if="multiple && showConfirmButton" class="zx-select__footer">
				<view class="zx-select__btn zx-select__btn--cancel" @click.stop="handleCancel">
					<text>{{ cancelButtonText }}</text>
				</view>
				<view class="zx-select__btn zx-select__btn--confirm" @click.stop="handleConfirm">
